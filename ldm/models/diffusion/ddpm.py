...
from Resnet_Classification import RSR
import torch
...
from basicsr.data.degradations import random_add_gaussian_noise_pt, random_add_poisson_noise_pt, \
    random_add_speckle_noise_pt, random_add_saltpepper_noise_pt, bivariate_Gaussian
import random
...
def visualize_fea(save_path, fea_img):
    fig = plt.figure(
        figsize=(fea_img.shape[1] / 10, fea_img.shape[0] / 10))  # Your image (W)idth and (H)eight in inches
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        print("unet", unet_config)
        print("con", conditioning_key)
...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def __init__(self,
        ...
        print('11111first_stage_config', first_stage_config)
        self.num_timesteps_cond = default(num_timesteps_cond, 1)
        self.scale_by_std = scale_by_std
        ...
        self.register_schedule(given_betas=np.array(new_betas), timesteps=len(new_betas),
                               linear_start=kwargs['linear_start'], linear_end=kwargs['linear_end'])
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
                  ...
                  resize_lq=True):
        r = batch['r'].cuda()
        # basicsr\data\realesrgan_dataset.py 中调用了判断空间分辨率的模块
        # r为空间分辨率 （数字2-10）
        im_gt = batch['gt'].cuda()
        h = im_gt.shape[-2]
        w = im_gt.shape[-1]
        ...
        im_gt = torch.squeeze(im_gt, dim=0)

        im_lr = self.resize_fn(im_gt, (lr, lr))
        im_lr = self.resize_fn(im_lr, (h, h))
        # 先下采样再上采样  保证lq gt大小一致
        im_lr = torch.clamp(im_lr, 0, 1.0)

        im_gt = im_gt.to(self.device)
        im_lr = im_lr.to(self.device)

        im_gt = torch.unsqueeze(im_gt, dim=0)
        im_lr = torch.unsqueeze(im_lr, dim=0)

        im_gt = im_gt / 255
        im_gt = im_gt * 2 - 1  # 归一化到（-1,1）

        im_lr = im_lr * 2 - 1
        im_lr = torch.clamp(im_lr, -1.0, 1.0)

        encoder_posterior_y = self.encode_first_stage(im_gt)

        z_gt = self.get_first_stage_encoding(encoder_posterior_y).detach()
        # encode_first_stage为配置文件中的first_stage  模型图中的黄色encoder
        encoder_posterior = self.encode_first_stage(im_lr)
        z = self.get_first_stage_encoding(encoder_posterior).detach()

        text_cond = [str(r.item() / 10)]
        # text为输入的条件 str 0.2-1.0
        ...
        if return_first_stage_outputs:
            ...
            out.extend([x, im_gt, xrec])
        if return_original_cond:
            out.append(xc)
            #  测试时需要输出的一些图片  训练out只有三项 lq text gt
        ...
        # print("lq,gt", self.lq.shape, self.gt.shape)
        #
        # if resize_lq:
    ...
    def forward(self, x, c, gt, *args, **kwargs):
        ...
        if self.test_gt:
            ...
            # lq经过structcond_stage_model得到条件struc_c  即论文中的time-aware-encoder
    ...
    def p_losses(self, x_start, cond, struct_cond, t, t_ori, z_gt, noise=None):
        # return self.p_losses(z_gt, c, sc_x, t, t_ori, z_lq, *args, **kwargs)
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        # 加噪
        if self.mix_ratio > 0:
    ...
    def p_mean_variance(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False,
    ...
    def p_mean_variance_canvas(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False,
        ...
        model_mean, posterior_variance, posterior_log_variance = self.q_posterior(x_start=x_recon, x_t=x,
                                                                                  t=t[:x_recon.size(0)])
        if return_codebook_ids:
            return model_mean, posterior_variance, posterior_log_variance, logits
        elif return_x0:
    ...
    def p_sample(self, x, c, struct_cond, t, clip_denoised=False, repeat_noise=False,
        ...
        outputs = self.p_mean_variance(x=x, c=c, struct_cond=struct_cond, t=t, clip_denoised=clip_denoised,
                                       ...
                                       reference_sr=reference_sr, reference_lr=reference_lr,
                                       reference_step=reference_step, reference_range=reference_range)
        if return_codebook_ids:
            raise DeprecationWarning("Support dropped.")
            model_mean, _, model_log_variance, logits = outputs
        elif return_x0:
    ...
    def p_sample_canvas(self, x, c, struct_cond, t, clip_denoised=False, repeat_noise=False,
                        return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                        temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, t_replace=None,
                        tile_size=64, tile_overlap=32, batch_size=4, tile_weights=None, unconditional_conditioning=None,
    ...
    def p_sample_loop(self, cond, struct_cond, shape, return_intermediates=False,
        ...
        for i in iterator:
            ...
            img = self.p_sample(img, cond, struct_cond_input, ts,
                                ...
                                unconditional_conditioning=unconditional_conditioning,
                                unconditional_guidance_scale=unconditional_guidance_scale,
                                reference_sr=reference_sr, reference_lr=reference_lr, reference_step=reference_step,
    ...
    def _gaussian_weights(self, tile_width, tile_height, nbatches):
        ...
        return torch.tile(torch.tensor(weights, device=self.betas.device),
    ...
    def p_sample_loop_canvas(self, cond, struct_cond, shape, return_intermediates=False,
                             x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
                             mask=None, x0=None, img_callback=None, start_T=None,
                             log_every_t=None, time_replace=None, adain_fea=None, interfea_path=None, tile_size=64,
                             tile_overlap=32, batch_size=4,
                             unconditional_conditioning=None, unconditional_guidance_scale=None,
                             reference_sr=None, reference_lr=0.05, reference_step=1, reference_range=[100, 1000], ):
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if sample:
            ...
            if quantize_denoised and not isinstance(self.first_stage_model, AutoencoderKL) and not isinstance(
                    self.first_stage_model, IdentityFirstStage):
                with self.ema_scope("Plotting Quantized Denoised"):
                    samples, z_denoise_row = self.sample_log(cond=c, struct_cond=struct_cond, batch_size=N,
                                                             ddim=use_ddim,
                                                             ddim_steps=ddim_steps, eta=ddim_eta,
        ...
        if plot_progressive_rows:
            with self.ema_scope("Plotting Progressives"):
                img, progressives = self.progressive_denoising(c, struct_cond=struct_cond,
                                                               shape=(self.channels, self.image_size // 8,
                                                                      self.image_size // 8),
                                                               batch_size=N)
            prog_row = self._get_denoise_row_from_list(progressives, desc="Progressive Generation")
            log["progressive_row"] = prog_row
