...
from Resnet_Classification import ResNet50, Bottleneck
...
class RealESRGANDataset(data.Dataset):
            ...
            if isinstance(opt['gt_path'], str):
                ...
                if len(opt['gt_path']) > 1:
                    for i in range(len(opt['gt_path']) - 1):
                        self.paths.extend(
                            sorted([str(x) for x in Path(opt['gt_path'][i + 1]).glob('*.' + opt['image_type'])]))
        if 'imagenet_path' in opt:
            class_list = os.listdir(opt['imagenet_path'])
            for class_file in class_list:
                self.paths.extend(
                    sorted([str(x) for x in Path(os.path.join(opt['imagenet_path'], class_file)).glob('*.' + 'JPEG')]))
            ...
            if isinstance(opt['face_gt_path'], str):
                ...
                if len(opt['face_gt_path']) > 1:
                    for i in range(len(opt['face_gt_path']) - 1):
                        self.paths.extend(
                            sorted([str(x) for x in Path(opt['face_gt_path'][0]).glob('*.' + opt['image_type'])])[
                            :opt['num_face']])
    ...
    def __getitem__(self, index):
        ...
        lr = hr // scale
        x0 = random.randint(0, img_gt.shape[-2] - hr)
        y0 = random.randint(0, img_gt.shape[-1] - hr)
        im_gt = img_gt[:, x0: x0 + hr, y0: y0 + hr]

        lr_img = self.resize_fn(im_gt, (lr, lr))
        lr_img = self.resize_fn(lr_img, (hr, hr))

        lr_img = torch.unsqueeze(lr_img, dim=0)

        with torch.no_grad():
            s = self.rsr(lr_img)
            _, r = torch.max(s, 1)

        return_d = {'gt': img_gt, 'r': r.item(), 'gt_path': gt_path}
        return return_d
